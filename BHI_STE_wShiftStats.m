function [STE, SEx, SEy, Len, Surr] = BHI_STE_wShiftStats(microS1_smth, RR, N_labels1, N_labels2, significance, shift)
% BHI_SymbolicTransferEntropy
    [STE, SEx, SEy, Len] = BHI_SymbolicTransferEntropy(microS1_smth, RR, N_labels1, N_labels2);
...
%             microS1_perm = microS1_smth(randperm(length(microS1_smth)));
            [STEp(pp,:), SExp(pp), SEyp(pp), ~] = BHI_SymbolicTransferEntropy(microS1_perm, RRperm, N_labels1, N_labels2);
        end
